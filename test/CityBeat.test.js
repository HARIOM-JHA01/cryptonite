...
describe('Contracts', () => {
  ...
  beforeEach(async () => {
    // const hardhatToken = await ethers.deployContract('Cryptonite')
    ;[owner] = await ethers.getSigners()
    contract = await ethers.deployContract('Cryptonite')
    await contract.waitForDeployment()
  })

  describe('Event', () => {
    beforeEach(async () => {
      const resultEvent = await contract
        .connect(owner)
    ...
    describe('Success', () => {
      ...
      it('Should confirm event update', async () => {
        ...
        const newAmount = 2.5
        await contract.connect(owner).updateEvent(id, name, description, image, location, amount)
        result = await contract.getEvent(id)
    ...
    describe('Failures', () => {
      it('Should confirm event creation failures', async () => {
        await expect(
          contract.connect(owner).createEvent('', description, image, location, amount)
        ).to.be.revertedWith('Name cannot be empty')

        await expect(
          contract.connect(owner).createEvent(name, description, image, location, 0)
        ).to.be.revertedWith('Amount cannot be zero')
      })

      it('Should confirm event update failures', async () => {
        await expect(
          contract.connect(owner).updateEvent(100, name, description, image, location, amount)
  ...
  describe('User', () => {
    ...
    describe('Failures', () => {
      it('Should confirm user creation failures', async () => {
        await expect(
          contract.connect(owner).createUser('', lastNme, userName, email, password, phone)
        ).to.be.revertedWith('First Name cannot be empty')
        await expect(
          contract.connect(owner).createUser(firstName, lastNme, userName, email, password, '')
        ).to.be.revertedWith('Phone cannot be empty')
        await expect(
          contract.connect(owner).createUser(firstName, lastNme, userName, email, '', phone)
